...
class QuestionAdapter(val context: Context?, private var questions: MutableList<Questions>) :
                            ...
                            query.addListenerForSingleValueEvent(object : ValueEventListener {
                                override fun onDataChange(dataSnapshot: DataSnapshot) {
                                    for (snapshot in dataSnapshot.children) {
                                        ...
                                        intent.putExtra(
                                            "cb4",
                                            snapshot.child("category").child("3").value.toString()
                                        )
                                        intent.putExtra(
                                            "cb5",
                                            snapshot.child("category").child("4").value.toString()
                                        )
                                        intent.putExtra(
                                            "cb6",
                                            snapshot.child("category").child("5").value.toString()
                                        )
                    ...
                    if (que > 0) {
                        val query: Query =
                            databaseReferencePaper.child(key[que - 1]).child("questions")
                                .orderByChild("question").equalTo(question.question)
                        query.addListenerForSingleValueEvent(object : ValueEventListener {
                            override fun onDataChange(dataSnapshot: DataSnapshot) {
