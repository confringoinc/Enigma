            ...
            if (!isNetworkAvailable()) {
                ...
                if (intent.hasExtra("paperKey")) {
                    answer?.let {
                        ...
                        databaseReference.child(answer.key!!).child("name").setValue(answer.name)
                            .addOnCompleteListener {
                                databaseReference.child(answer.key!!).child("marks")
                                    .setValue(answer.marks)
                                    .addOnCompleteListener {
                                        progressBar!!.visibility = View.GONE
                                        Toast.makeText(this, "Paper Edited", Toast.LENGTH_LONG)
                                            .show()
                                        resetComponents()
                                        onBackPressed()
                                    }
                    ...
                    answer?.let {
                        ...
                        val query: Query =
                            databaseReference.orderByChild("name").equalTo(answer.name)
