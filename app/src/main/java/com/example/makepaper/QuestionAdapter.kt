...
class QuestionAdapter(val context: Context?, private var questions: MutableList<Questions>): RecyclerView.Adapter<QuestionAdapter.MyViewHolder>(){
    ...
    val databaseReference = FirebaseDatabase.getInstance().reference.child(FirebaseAuth.getInstance().currentUser?.uid!!).child("questions")
        ...
        fun setData(question: Questions) {
            ...
            itemView.ib_options.setOnClickListener {
                ...
                popup.setOnMenuItemClickListener { item ->
                    when(item.title) {
                        "Edit" -> {
                            val query: Query = databaseReference.orderByChild("question").equalTo(question.question)
